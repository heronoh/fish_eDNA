---
title: "Evaluation of detection power of three 12S makers for 40 selected species of the Jequitinhonha & São Francisco Rivers Basins"
author: 
...
# # 10 - plot quality profiles ----
# cutFs
# cutRs
# 
# #paths to cutadapt output after primer removal
# #only NeoFish samples
# neo_fnRs <- cutRs[str_detect(string = sample.names, pattern = "Neo")]
# neo_fnFs <- cutFs[str_detect(string = sample.names, pattern = "Neo")]
# #only MiFish samples
# mif_fnFs <- cutFs[str_detect(string = sample.names, pattern = "Mi")]
# mif_fnRs <- cutRs[str_detect(string = sample.names, pattern = "Mi")]
# 
# 
# dev.off()
# 
# options(scipen=999)
# 
# plotQualityProfile(neo_fnFs[44]) #plot quality for all forward reads
# plotQualityProfile(neo_fnRs[]) #plot quality for all reverse reads
# 
# plotQualityProfile(mif_fnFs[]) #plot quality for all forward reads
# plotQualityProfile(mif_fnRs[]) #plot quality for all reverse reads
# 
```
...
LGC12Sdb <- Biostrings::readDNAStringSet(filepath ="/home/heron/prjcts/fish_eDNA/data/refs/db/LGC/fev21/decipher_tax_fullDB_order_fev21.fasta" )
##prepare DB
...
primers_n_samples
sample_info_tab <- primers_n_samples[,1:6] %>%  unique()
  # and setting the color column to be of type "character", which helps later
# sample_info_tab$color <- as.character(sample_info_tab$color)
asv_tab


sample_info_tab <- sample_info_tab %>% as.data.frame()

rownames(sample_info_tab) <- sample_info_tab$File_name

 # first we need to make a DESeq2 object
deseq_counts <- DESeqDataSetFromMatrix(countData = asv_tab, colDat = sample_info_tab, design = ~Group) 
...
dend_cols <- as.character(sample_info_tab$color[order.dendrogram(euc_dend)]) # tem que criar uma coluna de cores
...
vegan::rarecurve(t(asv_tab), step=100, 
          # col=sample_info_tab$color,
          lwd=2, ylab="ASVs", label=F)

  # and adding a vertical line at the fewest seqs in any sample
abline(v=(min(rowSums(t(asv_tab)))))
abline(v=15000)


=======
## and classifying
tax_info <- IdTaxa(test=dna, trainingSet=LGC12Sdb_trainingSet, strand="both", processors = NULL)



#alpha div
  # first we need to create a phyloseq object using our un-transformed count table
count_tab_phy <- otu_table(asv_tab, taxa_are_rows=T)
...
# 
# library("microbiome")
# microbiome::summarize_phyloseq(all_ps)
# 
# alpha_div <- all_ps %>% phyloseq::estimate_richness(split = T, measures = c("Observed", "Chao1", "Shannon")) %>%
#   mutate(File_name = sample_names(all_ps)) # add sample IDs
# 
# alpha_div <- right_join(microbiome::meta(all_ps), # add metadata
#                         alpha_div, by = "File_name")
# 
# # alpha_div <- left_join(alpha_div, primers_n_samples[,c(1,3)], by="File_name")
# 
# alpha_div_long <- reshape2::melt(alpha_div, measure.vars = c("Observed", "Chao1", "Shannon"))
# 
...
# 
# group <- levels(as.factor(alpha_div$Group))
# group <- group[c(9,10,14,16,17,18)]
# group.pairs <- combn(seq_along(group), 2, simplify = FALSE, FUN = function(i)group[i])
# 
#   library(ggpubr)
# colnames(alpha_div_long)
# 
# alpha_div_long %>% 
#   filter(Group %in% c("JqNnorm", "JqNorm", "Positive control", "SFJQ", "SFnNorm","SFNorm")) %>% 
# ggboxplot(x = "Group",y = "value",
#                              xlab = FALSE, ylab = FALSE, fill = "Group",
# # palette = c("gold2", "maroon4","royalblue4"),
# facet.by = "variable",
# scales = "free") +
# rremove("x.text") +
# rremove("x.ticks") +
...
Now we will count total ASVs and which were considered _in range_ or not for each sample
...
# 
# SFJQ_ps_tbl_blast <- all_ps_tbl_blast %>%
#   filter(Type %in% c("DNA pool","Positive control")) %>%
#   filter(Run %in% c("LGC_MiniSeq_1","LGC_MiniSeq_2")) %>% 
#   filter(Abundance > 0)
# 
# 
# SFJQ_ps_tbl_blast$`ASV (Sequence)` %>% unique()
# 
# 
# 
# #creat table with ASV counts per sample
# ASVs_per_sample <- SFJQ_ps_tbl_blast %>%
#      group_by(Sample) %>%
#    summarise("Run" = unique(Run),
#              "Group" = unique(Group),
#              "Primer" = unique(Primer),
#              "Total ASV" = length(unique(`ASV (Sequence)`[Abundance != 0])),
#              "ASVs in range" = length(unique(`ASV (Sequence)`[Abundance != 0 & `Expected length` == "in range"])),
#              "ASVs out of range" = length(unique(`ASV (Sequence)`[(`ASV (Sequence)`) != 0 & `Expected length` == "out of range"])),
#              "ABD" = sum(Abundance),
#              "UniqueSP" = length(unique(`final ID`[Abundance != 0 & `Expected length` == "in range"])))
# 
# ASVs_per_sample %>%  group_by(Primer) %>% 
#   summarize(
#     "SUM_Total ASV" = sum(`Total ASV`),
#     "SUM_ASVs in range" = sum(`ASVs in range`),
#     "SUM_ASVs out of range" = sum(`ASVs out of range`)) 
# 
# 
# 
# 
# SFJQ_ps_tbl_blast %>% filter(`Expected length` %in% c("in range")) %>% select(`ASV (Sequence)`, `final ID`)%>% unique()
...
#   ggplot2::ggplot(aes(x=str_remove(Sample,"Normalized "),
#                       y= Counts ,fill= `ID type`, group = `ID type`,col= Norm))+
...
#   geom_segment(aes(x=0.5, xend=2.5, y=40, yend=40,linetype="Number of original\ncomposing species"), colour="black",size=0.5) +
#   geom_segment(aes(x=2.5, xend=6.5, y=23, yend=23,linetype="Number of original\ncomposing species"), colour="black",size=0.5) +
#   geom_segment(aes(x=6.5, xend=12.5, y=18, yend=18,linetype="Number of original\ncomposing species"), colour="black",size=0.5) + 
#   guides(fill=guide_legend(title="Identification level")) +
#   scale_linetype_manual("",values=c("Number of original\ncomposing species"=3)) +
...
# 
# all_ps_tbl_sfjq_full %>%
#   # mutate(`Percentage on respective Non-norm pool` = sub("%","",`Percentage on respective Non-norm pool`)/100) %>%
#   # filter(`Expected Species` %in% c("expected")) %>%
#   filter(Run %in% c("LGC_MiniSeq_1","LGC_MiniSeq_2")) %>%
#   # filter(SF %in% c(TRUE)) %>%
#   filter(Sample %in% c(
...
#   pivot_longer(cols = c("Percentage on respective Norm pool", "Percentage on respective Non-norm pool" ),
#                names_to = "Pool",values_to = "Proportion") %>% 
#   mutate(Proportion = as.numeric(str_remove(Proportion,"%"))) %>% 
#   # # filter(Group %in% c("Normalized JQmc")) %>% select(Proportion) %>% unique()
#    ggplot(aes(x= log2(`Relative abundance on sample`/Proportion),y= `final ID`, fill = Primer, col = Pool)) +
#    geom_col(stat = "identity",position = "dodge")+
#   scale_color_manual(values = c("#00000f","#969595"),
#                      labels = c("Non-normalized","Normalized"))+
#   scale_fill_manual(
#     labels = c("NeoFish", "MiFish", "Teleo", "NeoFish/MiFish/Teleo", "NeoFish/MiFish"),
#                      values = alpha(colour = colors5 ,alpha =  0.8))+
#   xlab("Species") +
#   ylab("Proportion of ASVs in sampe= divided by proportion of DNA input") +
...
#gráficos de barras para as espécies
# 
#  all_ps_tbl_sfjq %>%  
#   filter(Run %in% c("LGC_MiniSeq_1","LGC_MiniSeq_2")) %>% 
#   filter(Sample %in% c(
...
#   )) %>% 
#   # filter(JQ==FALSE) %>% 
#   # filter(`Relative abundance on sample` >= 0.05) %>%
#   filter(!`final ID` %in% c("Poecilia reticulata mitochondr", "NA elegans/gilbert", 
#                             "NA","Coptodon zillii",NA , "Pseudoplatystoma corruscans", 
#                             "Acestrorhynchus lacustris", "Geophagus brasiliensis")) %>% 
#   ggplot(aes(y=rev(`final ID`),x=Abundance, fill=Primer))+
#   # ggchicklet::geom_chicklet(radius = grid::unit(1, 'mm'))
#   geom_col(color = "black",size=0.5,
#            position = position_dodge(width = 0.9)
#            # position = "dodge"
#            )+
#   xlab("ASV Abundance (log10)") +
#   ylab("Associated taxonomy") +
#   theme_light(base_size = 20)+
#   # coord_fixed(ratio = 1,expand = FALSE, xlim = c(-0.5,18))+
#   scale_fill_manual(
#     labels = c("NeoFish", "MiFish", "Teleo", "NeoFish/MiFish/Teleo", "NeoFish/MiFish"),
#                      values = alpha(colour = colors5 ,alpha =  0.8))+
#   annotate(geom = "rect",
#            xmin = 0, xmax = 24,
#            ymin = c(0.5,13.5,28.5,32.5),
#            ymax = c(1.5,14.5,29.5,33.5),
#            alpha=0.3,fill="yellow") 
#  # +
#  #  facet_wrap(Group~Primer,ncol = 3,drop = TRUE) 
# 
# 
# 
# 
...
#                             "NA","Coptodon zillii",NA , "Pseudoplatystoma corruscans", 
#                             "Acestrorhynchus lacustris", "Geophagus brasiliensis")) %>% 
#   ggplot(aes(y=rev(`final ID`),x=log10(Abundance), fill=Primer))+
#   # ggchicklet::geom_chicklet(radius = grid::unit(1, 'mm'))
...
# 
# # 
# ggsave(file = "~/prjcts/fish_eDNA/sfjq/results/figs/sfjq_ASVbar_SFJQ.pdf",
#      plot = SFJQ_asvs_bar,
#      device = "pdf",
#      width = 18,
#      height = 13,
...
# 
# # 
# ggsave(file = "~/prjcts/fish_eDNA/sfjq/results/figs/sfjq_ASVbar_SFJQ.png",
#      plot = SFJQ_asvs_bar,
#      device = "png",
#      width = 18,
#      height = 13,
...
#   ggplot(aes(y=rev(`final ID`),x=log10(Abundance), fill=Primer))+
#   # ggchicklet::geom_chicklet(radius = grid::unit(1, 'mm'))
#   geom_col(stat = "identity",color = "black",size=2,
#            # ,position = "dodge"
...
#   ylab("Associated taxonomy") +
#   theme_light(base_size = 20)+
#   coord_fixed(ratio = 1,expand = FALSE, xlim = c(-0.5,23))+
#   scale_fill_manual(
#     labels = c("NeoFish", "MiFish", "Teleo", "NeoFish/MiFish/Teleo", "NeoFish/MiFish"),
#                      values = alpha(colour = colors5 ,alpha =  0.8))
#   # annotate(geom = "rect",
#   #          xmin = 0, xmax = 24,
#   #          ymin = c(0.5,7.5,9.5,13.5,18.5,20.5),
#   #          ymax = c(1.5,8.5,10.5,14.5,19.5,21.5),
#   #          alpha=0.3,fill="yellow")
#   #   +
#   # facet_wrap(Primer~Group,ncol = 2,drop = TRUE)
# 
# JQ_asvs_bar
# # 
# # 
# # # 
# # 
# # ggsave(file = "~/prjcts/fish_eDNA/sfjq/results/figs/sfjq_ASVbar_JQ.pdf",
# #      plot = JQ_asvs_bar,
# #      device = "pdf",
# #      width = 24,
# #      height = 17,
...
# # 
# # 
# # ggsave(file = "~/prjcts/fish_eDNA/sfjq/results/figs/sfjq_ASVbar_JQ.png",
# #      plot = JQ_asvs_bar,
# #      device = "png",
# #      width = 24,
# #      height = 17,
...
# 
# 
# SF_asvs_bar <- all_ps_tbl_sfjq_full %>%  
#   filter(Run %in% c("LGC_MiniSeq_1","LGC_MiniSeq_2")) %>% 
#   filter(Sample %in% c(
# # # "Non-normalized JQmc\nMiFish","Non-normalized JQmc\nNeoFish", "Non-normalized JQmc\nTeleo",
# # "Normalized JQmc\nMiFish","Normalized JQmc\nNeoFish","Normalized JQmc\nTeleo"
...
#   # filter(JQ==FALSE) %>% 
#   # filter(`Relative abundance on sample` >= 0.05) %>%
#   filter(!`final ID` %in% c("Poecilia reticulata mitochondr", "NA elegans/gilbert", "NA lepidura/xenodon", 
#                             "NA","Coptodon zillii",NA , "Pseudoplatystoma corruscans", 
#                             "Acestrorhynchus lacustris", "Geophagus brasiliensis",
#                             "Astyanax lacustris")) %>% 
#   ggplot(aes(y=rev(`final ID`),x=log10(Abundance), fill=Primer))+
...
#            ymax = c(10.5,21.5),
#            alpha=0.3,fill="yellow") +
#   facet_wrap(Group~Primer,ncol = 2,drop = TRUE)  
# 
# SF_asvs_bar
# 
# 
# # 
# ggsave(file = "~/prjcts/fish_eDNA/sfjq/results/figs/sfjq_ASVbar_SF.pdf",
#      plot = SF_asvs_bar,
#      device = "pdf",
#      width = 18,
#      height = 20,
...
# 
# # 
# ggsave(file = "~/prjcts/fish_eDNA/sfjq/results/figs/sfjq_ASVbar_SF.png",
#      plot = SF_asvs_bar,
#      device = "png",
#      width = 18,
#      height = 20,
...
# ggsave(file = "~/prjcts/fish_eDNA/sfjq/data/trees/JQmc-23_SPs_fused-unique-RRA_barplot.pdf", plot = JQmc_RRA_plot, device = "pdf", width = 24, height = 20, units = "cm", dpi = 600)
# ggsave(file = "~/outros/sfjq_temp/trees/sfjq_foldchange_boxplot.pdf", plot = fold_change_boxplot, device = "pdf", width = 40, height = 20, units = "cm", dpi = 600)
ggsave(file = "/home/heron/prjcts/fish_eDNA/sfjq/results/figs/jun22_sfjq_foldchange_boxplot.pdf", plot = fold_change_boxplot, device = "pdf", width = 40, height = 24, units = "cm", dpi = 600)
...
# Seqs2DB(seq = "/home/heron/prjcts/fish_eDNA/",
...
writeXStringSet(x = dna_order,
                filepath = "/home/heron/prjcts/fish_eDNA/data/refs/db/DB_order_clust.fasta",
                format = "fasta")
...
#write fasta
writeXStringSet(x = final_dna_clean,filepath = "/home/heron/prjcts/fish_eDNA/data/refs/db/LGC/fev21/dada_tax_fullDB_order_fev21.fasta",format = "fasta")
...
writeXStringSet(x = final_dna_clean,filepath = "/home/heron/prjcts/fish_eDNA/data/refs/db/LGC/fev21/dada_tax_fullDB_order_SPs_fev21.fasta",format = "fasta")
...
###############################################
                                                                       20        
                                                     '''''''''|'''''''''|        
                                          neo_FWD    CGCCGTCGCAAGCTTACCCT    20    
               SF | Synbranchus marmoratus | 1404    ········TC·········C    20    
               SF | Synbranchus marmoratus | 1405    ········TC·········C    20    
               Jq | Oligosarcus macrolepis | 7690    ·A·················C    20    
              SF | Leporinus macrocephalus | 0982    ···················C    20    
    SFJq2 | Conorhynchos conirostris | JX899737.1    ·A······TC·········C    20    
             Jq | Rhamdia cf jequitinhonha | 3054    ········TC·········C    20    
              SF | Cetopsorhamdia iheringi | 1372    ········TC·········C    20    
                   SF | Imparfinis minutus | 1161    ········TC·········C    20    #
                   SF | Imparfinis minutus | 1370    ········TC·········C    20    
                       SF | Rhamdia quelen | 1361    ·······AT······G····    20    
                   Jq | Rhamdia aff quelen | 5645    ········T······G····    20    #
                  SF | Pimelodella vittata | 1287    ···············G····    20    
                  SF | Pimelodella vittata | 1288    ···············G····    20    
                  SF | Pimelodella vittata | 1289    ···············G····    20    
          SF | Pseudoplatystoma corruscans | 1135    ········TC·········C    20    
          Jq | Steindachneridion amblyurum | 2996    ········TC·········C    20    #
                  SF | Pimelodus maculatus | 0749    ········TC·········C    20    
                        SF | Pimelodus fur | 1147    ········TC·········C    20    
                  SF | Pimelodus maculatus | 1280    ········TC·········C    20    #
                        SF | Pimelodus fur | 1345    ········TC·········C    20    
                      SF | Pimelodus pohli | 1400    ········TC·········C    20    
                      SF | Pimelodus pohli | 1401    ········TC·········C    20    
                      SF | Pimelodus pohli | 1403    ········TC·········C    20    #
            SF | Microglanis leptostriatus | 1102    ········TC·········C    20    
            SF | Microglanis leptostriatus | 1104    ········TC·········C    20    #
                   SF | Pseudopimelodus sp | 0671    ········TC·········C    20    
                                                             
                                        Consensus    CRCCGTCGYMAGCTTRCCCY    20 
###############################################
###############################################
                                                             20      
                                           '''''''''|'''''''''|      
...
###############################################
                                                     ...
                                                     '''''''''|'''''''''|        
                                          neo_FWD    CGCCGTCGCAAGCTTACCCT    20    
               Out | Prionace glauca | AF448008.1    ········TC····C·····    20    
               SF | Synbranchus marmoratus | 1404    ········TC·········C    20    
               SF | Synbranchus marmoratus | 1405    ········TC·········C    20    
                 Jq | Eugerres brasilianus | 2943    ····················    20    
                Jq | Geophagus brasiliensis | 303    ········TC··········    20    
                Jq | Geophagus brasiliensis | 304    ········TC··········    20    
                     Jq | Australoheros sp | 6584    ········TC··········    20    
                 SF | Crenicichla lepidota | 1129    ········TC··········    20    
                 SF | Crenicichla lepidota | 1130    ········TC··········    20    
                 SF | Crenicichla lepidota | 1264    ········TC··········    20    
                      SF | Phalloceros uai | 1368    ········TC···A······    20    
                      SF | Phalloceros uai | 1369    ········TC···A······    20    
          Jq | Acinocheirodon melanogramma | 1547    ········TC···A······    20    
                       Jq | Phalloceros sp | 1551    ········TC···A······    20    
              SF | Pamphorichthys hollandi | 1098    ········TC···A······    20    
              SF | Pamphorichthys hollandi | 1099    ········TC···A······    20    
              Jq | Lycengraulis grossidens | 2918    ·A·······C··········    20    
           Jq | Anchoviella lepidentostole | 2912    ·A·······C··········    20    
            SF | Orthospinus franciscensis | 1071    ··············C·····    20    
                   SF | Moenkhausia costae | 1074    ··············C·····    20    
                   Jq | Moenkhausia costae | 7812    ··············C·····    20    
         SF | Moenkhausia sanctaefilomenae | 1111    ····················    20    
         SF | Moenkhausia sanctaefilomenae | 1112    ····················    20    
         SF | Moenkhausia sanctaefilomenae | 1113    ····················    20    
               SF | Hemigrammus marginatus | 1382    ····················    20    
              SF | Hemigrammus cf gracilis | 1380    ····················    20    
                SF | Hyphessobrycon santae | 1209    ····················    20    
                SF | Hyphessobrycon santae | 1210    ····················    20    
                SF | Hyphessobrycon santae | 1211    ····················    20    
            SF | Orthospinus franciscensis | 1072    ····················    20    
                   SF | Astyanax lacustris | 1239    ····················    20    
                Jq | Astyanax cf lacustris | 5672    ····················    20    
                   Jq | Astyanax lacustris | 7880    ····················    20    
               Jq | Astyanax aff fasciatus | 5695    ····················    20    
               Jq | Oligosarcus macrolepis | 7690    ·A·················C    20    
                SF | Astyanax cf fasciatus | 1044    ····················    20    
                SF | Astyanax cf fasciatus | 1048    ····················    20    
                SF | Astyanax cf fasciatus | 1153    ····················    20    
               Jq | Astyanax aff fasciatus | 5693    ····················    20    
               Jq | Astyanax aff fasciatus | 5694    ····················    20    
                    SF | Bryconops affinis | 1398    ········T···········    20    
                    SF | Bryconops affinis | 1399    ········T···········    20    
                      SF | Gymnotus carapo | 1248    ········TC··········    20    
                      Jq | Gymnotus carapo | 1610    ········TC··········    20    
                      Jq | Gymnotus carapo | 1631    ········TC··········    20    
                 SF | Sternopygus macrurus | 1121    ········TC··········    20    
                 SF | Sternopygus macrurus | 1124    ········TC··········    20    
                SF | Eigenmannia virescens | 1117    ·········C··········    20    
                SF | Eigenmannia virescens | 1118    ·········C··········    20    
                SF | Eigenmannia virescens | 1119    ·········C··········    20    
                      Jq | Delturus brevis | 1936    ········TC··········    20    
                    SF | Trichomycterus sp | 1373    ········TC··········    20    
                    Jq | Harttia garavelloi | 439    ········TC··········    20    
                           SF | Harttia sp | 1145    ········TC··········    20    
                           SF | Harttia sp | 1156    ········TC··········    20    
                         SF | Hisonotus sp | 1108    ·A······TC··········    20    
                         SF | Hisonotus sp | 1109    ·A······TC··········    20    
              Jq | Pareiorhaphis stephanus | 4196    ········TC··········    20    
                        Jq | Hypostomus sp | 5704    ·········C··········    20    
       SF | Pterygoplichthys etentaculatus | 1030    ·········C··········    20    
       SF | Pterygoplichthys etentaculatus | 1141    ·········C··········    20    
                    SF | Hypostomus alatus | 0925    ·········C··········    20    
                    SF | Hypostomus alatus | 1041    ·········C··········    20    
                        Jq | Hypostomus sp | 1711    ·········C··········    20    
             Jq | Hypostomus nigrolineatus | 7893    ·········C··········    20    
                        Jq | Hypostomus sp | 5650    ·········C··········    20    
                        Jq | Hypostomus sp | 5703    ·········C··········    20    
                 SF | Serrasalmus brandtii | 0872    ····················    20    
                   SF | Pygocentrus piraya | 1229    ····················    20    
                 SF | Serrasalmus brandtii | 1230    ····················    20    
                 Jq | Serrasalmus brandtii | 7890    ····················    20    
                 Jq | Serrasalmus brandtii | 6261    ····················    20    
       SF | Pterygoplichthys etentaculatus | 0861    ····················    20    
                  SF | Hoplias intermedius | 1377    ····················    20    
                 Jq | Hoplias brasiliensis | 1772    ····················    20    
                  SF | Hoplias malabaricus | 1234    ········T···········    20    
                  SF | Hoplias malabaricus | 1346    ········T···········    20    
                           Jq | Hoplias sp | 6630    ········T···········    20    
                  Jq | Hoplias malabaricus | 7822    ········T···········    20    
            SF | Acestrorhynchus lacustris | 0917    ····················    20    
            SF | Acestrorhynchus lacustris | 1136    ····················    20    
                    SF | Roeboides xenodon | 0916    ····················    20    
         SF | Phenacogaster franciscoensis | 1333    ····················    20    
                      SF | Odontostilbe sp | 1076    ····················    20    
                      SF | Odontostilbe sp | 1077    ····················    20    
                    SF | Serrapinnus piaba | 1086    ····················    20    
                    SF | Serrapinnus piaba | 1087    ····················    20    
                      SF | Odontostilbe sp | 1313    ····················    20    
            SF | Bryconamericus stramineus | 1150    ····················    20    
            SF | Bryconamericus stramineus | 1151    ····················    20    
            SF | Bryconamericus stramineus | 1152    ····················    20    
                     SF | Piabina argentea | 1306    ····················    20    
                     SF | Piabina argentea | 1307    ····················    20    
                    SF | Planaltina myersi | 1367    ····················    20    
         SFJq2 | Knodus moenkhausii  | KF209618.1    ····················    20    
             SF | Tetragonopterus chalceus | 0901    ····················    20    
             SF | Tetragonopterus chalceus | 0984    ····················    20    
                SF | Apareiodon ibitiensis | 1371    ····················    20    
                      SF | Parodon hilarii | 1245    ····················    20    
                      SF | Parodon hilarii | 1332    ····················    20    
                       SF | Characidium sp | 1308    ·············C······    20    
                       SF | Characidium sp | 1334    ·············C······    20    
                    SF | Characidium zebra | 1060    ····················    20    
                    SF | Characidium zebra | 1335    ····················    20    
             SF | Characidium lagosantense | 1379    ····················    20    
             SF | Characidium lagosantense | 1381    ····················    20    
                   SF | Brycon orthotaenia | 1005    ····················    20    
                   SF | Brycon orthotaenia | 1037    ····················    20    
           Jq | Hypomasticus steindachneri | 2984    ····················    20    
           Jq | Hypomasticus steindachneri | 5664    ····················    20    
           Jq | Hypomasticus steindachneri | 7826    ····················    20    
                       SF | Leporinus piau | 0958    ····················    20    
                  SF | Leporinus taeniatus | 0985    ····················    20    
                       SF | Leporinus piau | 0986    ····················    20    
                  SF | Leporinus taeniatus | 1317    ····················    20    
                       SF | Leporinus piau | 1384    ····················    20    
                  Jq | Leporinus taeniatus | 1553    ····················    20    
                     SF | Schizodon knerii | 0953    ····················    20    
                     SF | Schizodon knerii | 1003    ····················    20    
                     SF | Schizodon knerii | 1337    ····················    20    
              SF | Leporinus macrocephalus | 0982    ···················C    20    
                 SF | Leporinus reinhardti | 1344    ····················    20    
                Jq | Megaleporinus garmani | 4290    ····················    20    
                  SF | Leporinus elongatus | 0772    ····················    20    
                  SF | Leporinus elongatus | 1032    ····················    20    
                  SF | Leporinus elongatus | 1033    ····················    20    
              Jq | Megaleporinus elongatus | 1762    ····················    20    
              Jq | Megaleporinus elongatus | 1761    ····················    20    
              Jq | Megaleporinus elongatus | 6633    ····················    20    
                 Jq | Colossoma macropomum | 2871    ····················    20    
                        SF | Myleus micans | 0763    ····················    20    
                        SF | Myleus micans | 1162    ····················    20    
                 SF | Curimatella lepidura | 0900    ········T···········    20    
                    SF | Roeboides xenodon | 0905    ········T···········    20    
                 SF | Curimatella lepidura | 0906    ········T···········    20    
                 SF | Curimatella lepidura | 0907    ········T···········    20    
                  SF | Cyphocharax gilbert | 1054    ········T···········    20    
                  SF | Cyphocharax gilbert | 1056    ········T···········    20    
              SF | Steindachnerina elegans | 1284    ········T···········    20    
               Jq | Steindachnerina elegans | 231    ········T···········    20    
              Jq | Steindachnerina elegans | 6586    ········T···········    20    
                 SF | Prochilodus costatus | 0664    ····················    20    
                SF | Prochilodus argenteus | 0739    ····················    20    
                SF | Prochilodus argenteus | 0742    ····················    20    
                SF | Prochilodus argenteus | 0779    ····················    20    
                 SF | Prochilodus costatus | 1281    ····················    20    
                 SF | Prochilodus costatus | 1339    ····················    20    
                   Jq | Prochilodus hartii | 1765    ····················    20    
                 Jq | Prochilodus costatus | 2860    ····················    20    
                Jq | Prochilodus argenteus | 5612    ····················    20    
                   Jq | Prochilodus hartii | 7033    ····················    20    
          Out | Prochilodus costatus | NC027690.1    ····················    20    
                         Jq | Corydoras sp | 1568    ·A·······C··········    20    
               SF | Hoplosternum littorale | 1215    ········TC··········    20    
               SF | Hoplosternum littorale | 1274    ········TC··········    20    
               Jq | Hoplosternum littorale | 2643    ········TC··········    20    
             Jq | Rhamdia cf jequitinhonha | 3054    ········TC·········C    20    
                       SF | Rhamdia quelen | 1361    ·······AT······G····    20    
                   Jq | Rhamdia aff quelen | 5645    ········T······G····    20    
                  SF | Pimelodella vittata | 1287    ···············G····    20    
    ...
    SFJq2 | Conorhynchos conirostris | JX899737.1    ·A······TC·········C    20    
          ...
          SF | Pseudoplatystoma corruscans | 1135    ········TC·········C    20    
            ...
            SF | Microglanis leptostriatus | 1104    ········TC·········C    20    
                   SF | Pseudopimelodus sp | 0671    ········TC·········C    20    
                      SF | Bunocephalus sp | 1158    ········TC··········    20    
                      SF | Bunocephalus sp | 1159    ········TC··········    20    
            SF | Franciscodoras marmoratus | 0708    ········TC··········    20    
                Jq | Wertheimeria maculata | 7906    ········TC··········    20    
                Jq | Wertheimeria maculata | 7817    ········TC··········    20    
             SF | Trachelyopterus galeatus | 0602    ········TC··········    20    
             SF | Trachelyopterus galeatus | 1243    ········TC··········    20    
          Jq | Trachelyopterus cf galeatus | 5675    ········TC··········    20    
           Jq | Trachelyopterus striatulus | 6583    ········TC··········    20    
                                                             
                                        Consensus    CGCCGTCGYMAGCWTACCCY    20    
